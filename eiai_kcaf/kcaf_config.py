...
class KcafClusterConfig(ClusterConfig):
    def switch_cluster(self, cluster: ClusterEnum):
        if cluster not in ['unknown', 'none']:
            kubeconfig_prefix = os.environ["EIAI_KCAF_KUBECONFIG_PREFIX"]
            config = json.loads(os.environ["EIAI_KCAF_CLUSTER_KUBECONFIG"])
...
class KcafServiceConfig(ServiceConfig):
    def __init__(self):
        ...
        product_logging_config = os.environ.get('EIAI_KCAF_PRODUCT_LOGGING_CONFIG_LOCATION')
        ...
        kcaf_service_config = os.environ.get('EIAI_KCAF_SERVICE_CONFIG_LOCATION')
        if kcaf_service_config is None or kcaf_service_config == '':
            defautl_slack_channel = os.environ.get('EIAI_KCAF_DEFAULT_SLACK_CHANNEL')
            ...
            kcaf_services = os.environ.get("EIAI_KCAF_SERVICES")
            if kcaf_services is not None and kcaf_services != '':
                for service in os.environ.get("EIAI_KCAF_SERVICES").split(','):
        ...
        user_defined_config = os.environ.get('EIAI_KCAF_USER_DEFINED_CONFIG_LOCATION')
